...
foreach ( $reports as $report_name => $report ) : ?>
	<div class="gg_woo_feed-system-info-section gg_woo_feed-system-info-<?php echo esc_attr( $report_name ); ?>">
		<table class="widefat">
			<thead>
			<tr>
			...
			foreach ( $report['report'] as $field_name => $field ) :
				if ( in_array( $report_name, [ 'plugins', 'network_plugins', 'mu_plugins' ], true ) ) {
					...
					$warning_class = ! empty( $field['warning'] ) ? ' class="gg_woo_feed-warning"' : '';
