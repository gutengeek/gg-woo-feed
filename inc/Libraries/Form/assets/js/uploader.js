...
// Back-compat
window.gg_woo_feed_uploader = function() {
    return true;
};
...
( function( $, window ) {
    ...
    function gg_woo_feed_uploader() {
        ...
        function check_number_files( file , i   ) {
            if( is_image_file(file) ) {
                if( i+1 > gg_woo_feedJS.mfile_image ){
                    return false;
                }
            } else {
                if( i+1 > gg_woo_feedJS.mfile_other ){
                    return false;
                }
            }
            return true;
        ...
        function check_filesize (  file , i  ) {
            ...
            if( is_image_file(file) ) {
                if( file.size > gg_woo_feedJS.size_image ){
                    var myToast = $.toast({
                        heading: file.name,
                        text: gg_woo_feedJS.error_upload_size,
                        icon: 'error',
        ...
        function trigger_button_upload(){
              ...
              //  var all_selected = [];
               ...
               file_btn.on("change", function( event ){
                ...
                $( _container ).on( "click", ".btn-close", function(){
                    if( confirm(gg_woo_feedJS.confirmed ) ){
            ...
            $(".gg_woo_feed-uploader-files").each( function(){
                handleUpload( this )
            } );
    ...
    /** @namespace wp */
    window.wp = window.wp || {};
    window.wp.gg_woo_feed_uploader = gg_woo_feed_uploader();
